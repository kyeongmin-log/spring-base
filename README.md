...
스프링 데이터 JPA는 JPA를 편리하게 쓸 수 있도록 도와주는 기술이다. 따라서, JPA를 먼저 배우고 배우는 것 좋다.

JpaRepository 인터페이스로 가보면 기본적으로 CRUD 기능들이 구현되어 있다. 따라서, 따로 구현하지 않고 인터페이스만 만들어 놓아도 사용이 가능하다.
